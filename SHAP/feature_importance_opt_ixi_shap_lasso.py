...
def main():
    ...
    # k-fold CV params
    skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=1) # n_split : 5
    ...
    opt_lasso_feature_num = 46
    ixi_lasso = ixi_exp.loc[:, ['Feature','Lasso']]
    ixi_lasso_sort = ixi_lasso.sort_values(by='Lasso', ascending=False)
    
    
    ixi_lasso_feat_list = ixi_lasso_sort.Feature.to_list()
    ixi_lasso_features = ixi_lasso_feat_list[:opt_lasso_feature_num]
    ixi_lasso_features.append('Age')
    ixi_lasso_features.append('Subject')
 
    subject_data = subject_data.loc[:, ixi_lasso_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
    # 데이터 셋을 변경하는 경우 이 부분 변경 필요
    n_subs = 453
    n_features = opt_lasso_feature_num
    num_of_models = 1
    num_folds = 10
    ...
    print('---------------------------------------------------------')
    # collate data
    preds = pd.DataFrame(np.hstack((preds, uncorr_preds)), columns = ['Lasso_preds', 'Lasso_uncorr_preds'])
    fold = pd.DataFrame(fold.astype(int), columns=['fold'])
    predictions = pd.concat((subject_data, fold, preds), axis=1)
 
    # saving
    print('model predictions: {:}revision_shap_cv10/model_predictions-.csv'.format(outpath))
    print('')
    predictions.to_csv('{:}revision_shap_cv10/{:}{:}_predictions.csv'.format(outpath, data_name, model_name), index=False)
    ...
    fold_mae = pd.DataFrame(fold_mae, columns=['Lasso', 'Lasso_uncorr'])
    fold_mae.insert(0, 'fold', np.unique(predictions.fold))
 
    fold_r2 = pd.DataFrame(fold_r2, columns=['Lasso', 'Lasso_uncorr'])
    ...
    fold_mae.to_csv('{:}revision_shap_cv10/{:}{:}-MAE.csv'.format(outpath, data_name, model_name), index=False)
    # print('model accuracy (R2): {:}R2-{:}-{:}-{:}-{:}.csv'.format(outpath, run_combat, regress, run_pca, parc))
    fold_r2.to_csv('{:}revision_shap_cv10/{:}{:}-R2.csv'.format(outpath, data_name, model_name), index=False)
    print('')
 
    # explanations
    # 모델 변경시 이 부분 변경필요 
    for m, model_name in enumerate(['Lasso']):
        exp = pd.DataFrame(feature_explanations[m])
        fold = pd.DataFrame(fold.astype(int), columns=['fold'])
        feat_exp = pd.concat((subject_data, fold, exp), axis=1)
        print('model explanations: {:}revision_shap_cv10/{:}{:}-model-feature-explanations.csv'.format(outpath, data_name, model_name))
        print('')
        feat_exp.to_csv('{:}revision_shap_cv10/{:}-{:}-model-feature-explanations.csv'.format(outpath, data_name, model_name), index=False)
 
        # save for later CV models
        print('model explanations for cross-validation: {:}revision_shap_cv10/{:}-{:}-model-all-fold-feature-explanations.npy'.format(genpath, data_name, model_name))
        np.save('{:}revision_shap_cv10/{:}-{:}-model-all-fold-feature-explanations.npy'.format(genpath,data_name, model_name), fold_feature_explanations[m,:,:,:])
        print('model predictions for cross-validation: {:}revision_shap_cv10/{:}-{:}-model-all-fold-delta.npy'.format(genpath,data_name, model_name))
        np.save('{:}revision_shap_cv10/{:}-{:}-model-all-fold-delta.npy'.format(genpath, data_name, model_name), fold_predictions[m,:,:])
