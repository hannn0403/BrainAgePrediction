...
def main():
    ...
    cc_lasso = cc_exp.loc[:, ['Feature','Lasso']]
    cc_lasso_sort = cc_lasso.sort_values(by='Lasso', ascending=False)
    cc_lasso_feat_list = cc_lasso_sort.Feature.to_list()
    opt_lasso_feature_num = 64
    cc_lasso_features = cc_lasso_feat_list[:opt_lasso_feature_num]
    cc_lasso_features.append('Age')
    cc_lasso_features.append('Subject')

    subject_data = subject_data.loc[:, cc_lasso_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
