...
subject_data = pd.read_csv('./data/ixi_train.csv')
n_subs = 453
n_features = 153
num_of_models = 3
num_folds = 5
...
ixi_gbm = ixi_exp.loc[:, ['Feature','Gradient Boosting Regressor']]
...
ixi_gbm_sort = ixi_gbm.sort_values(by='Gradient Boosting Regressor', ascending=False)

ixi_lasso_feat_list = ixi_lasso_sort.Feature.to_list()
ixi_gpr_feat_list = ixi_gpr_sort.Feature.to_list()
ixi_gbm_feat_list = ixi_gbm_sort.Feature.to_list()
...
for feature_num in range(1, len(ixi_lasso_feat_list) + 1):
    if feature_num % 10 ==0:
        print(f"Lasso Using {feature_num} features") 
    # for문을 돌면서 Mean Absolute SHAP value가 가장 높은 순서대로 하나씩 추가해가며 
    # Model Type 변경시 이 부분 수정 
    subject_data_iter = subject_data.loc[:, ixi_lasso_feat_list[:feature_num]]
...
ixi_lasso_metrics = {'uncorr_mae' : uncorr_mae_list, 'uncorr_r2': uncorr_r2_list, 
                    'corr_mae':corr_mae_list, 'corr_r2': corr_r2_list}
ixi_lasso_metrics = pd.DataFrame(ixi_lasso_metrics)

ixi_lasso_metrics.to_csv('./ixi_lasso_metrics.csv')
print("IXI Dataset Lasso Complete!")
print('===========================================================')
...
for feature_num in range(1, len(ixi_gpr_feat_list) + 1):
    #if feature_num % 10 ==0:
    print(f"Gaussian Process Using {feature_num} features") 
    # for문을 돌면서 Mean Absolute SHAP value가 가장 높은 순서대로 하나씩 추가해가며 
    # Model Type 변경시 이 부분 수정 
    subject_data_iter = subject_data.loc[:, ixi_gpr_feat_list[:feature_num]]
...
ixi_gpr_metrics = {'uncorr_mae' : uncorr_mae_list, 'uncorr_r2': uncorr_r2_list, 
                    'corr_mae':corr_mae_list, 'corr_r2': corr_r2_list}
ixi_gpr_metrics = pd.DataFrame(ixi_gpr_metrics)
ixi_gpr_metrics.to_csv('./ixi_gpr_metrics.csv')
print("IXI GPR Model Complete!")
print('===========================================================')
...
for feature_num in range(1, len(ixi_gbm_feat_list) + 1):
    if feature_num % 10 ==0:
        print(f"Gradient Boosting Machine Using {feature_num} features") 
    # for문을 돌면서 Mean Absolute SHAP value가 가장 높은 순서대로 하나씩 추가해가며 
    # Model Type 변경시 이 부분 수정 
    subject_data_iter = subject_data.loc[:, ixi_gbm_feat_list[:feature_num]]
...
ixi_gbm_metrics = {'uncorr_mae' : uncorr_mae_list, 'uncorr_r2': uncorr_r2_list, 
                    'corr_mae':corr_mae_list, 'corr_r2': corr_r2_list}
ixi_gbm_metrics = pd.DataFrame(ixi_gbm_metrics)
ixi_gbm_metrics.to_csv('./ixi_gbm_metrics.csv')
