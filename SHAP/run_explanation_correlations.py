...
def main():
    ...
    for model in ['linear', 'nonlinear', 'ensemble']:
        ...
        explanations = model_explanations.loc[:,~model_explanations.columns.isin(['Subject', 'Age', 'Sex', 'fold'])]
        ...
        for sub in tqdm(np.arange(length), desc='subjects'):
            ...
            notsub = np.ones(length, dtype=bool)
            notsub[sub]=False
