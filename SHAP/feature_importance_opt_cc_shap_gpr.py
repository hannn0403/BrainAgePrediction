...
def main():
    ...
    cc_gpr = cc_exp.loc[:, ['Feature','Gaussian Process']]
    cc_gpr_sort = cc_gpr.sort_values(by='Gaussian Process', ascending=False)
    cc_gpr_feat_list = cc_gpr_sort.Feature.to_list()
    opt_gpr_feature_num = 83
    cc_gpr_features = cc_gpr_feat_list[:opt_gpr_feature_num]
    cc_gpr_features.append('Age')
    cc_gpr_features.append('Subject')

    subject_data = subject_data.loc[:, cc_gpr_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
