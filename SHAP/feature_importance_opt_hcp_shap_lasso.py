...
def main():
    ...
    subject_data = pd.read_csv(datapath + 'hcp_train.csv')
    ...
    opt_lasso_feature_num = 32
    hcp_lasso_features = hcp_lasso_feat_list[:opt_lasso_feature_num]
    hcp_lasso_features.append('Age')
    hcp_lasso_features.append('Subject')

    subject_data = subject_data.loc[:, hcp_lasso_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
    # 데이터 셋을 변경하는 경우 이 부분 변경 필요 
