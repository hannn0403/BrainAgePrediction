...
def main():
    ...
    cc_gbm = cc_exp.loc[:, ['Feature','Gradient Boosting Regressor']]
    cc_gbm_sort = cc_gbm.sort_values(by='Gradient Boosting Regressor', ascending=False)
    cc_gbm_feat_list = cc_gbm_sort.Feature.to_list()
    opt_gbm_feature_num = 54
    cc_gbm_features = cc_gbm_feat_list[:opt_gbm_feature_num]
    cc_gbm_features.append('Age')
    cc_gbm_features.append('Subject')

    subject_data = subject_data.loc[:, cc_gbm_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
    ...
    # 데이터 셋을 변경하는 경우 이 부분 변경 필요 
    n_subs = 500
    n_features = opt_gbm_feature_num
    num_of_models = 1
    num_folds = 5
    ...
    print('---------------------------------------------------------')
    # collate data
    preds = pd.DataFrame(np.hstack((preds, uncorr_preds)), columns = ['GBM_preds','GBM_uncorr_preds'])
    fold = pd.DataFrame(fold.astype(int), columns=['fold'])
    predictions = pd.concat((subject_data, fold, preds), axis=1)

    # saving
    print('model predictions: {:}revision_shap/{:}_predictions.csv'.format(outpath, model_name))
    print('')
    predictions.to_csv('{:}revision_shap/{:}_predictions.csv'.format(outpath, model_name), index=False)
    ...
    fold_mae = pd.DataFrame(fold_mae, columns=['GBM', 'GBM_uncorr'])
    fold_mae.insert(0, 'fold', np.unique(predictions.fold))

    fold_r2 = pd.DataFrame(fold_r2, columns=['GBM', 'GBM_uncorr'])
    ...
    fold_mae.to_csv('{:}revision_shap/{:}-MAE.csv'.format(outpath,model_name), index=False)
    # print('model accuracy (R2): {:}R2-{:}-{:}-{:}-{:}.csv'.format(outpath, run_combat, regress, run_pca, parc))
    fold_r2.to_csv('{:}revision_shap/{:}-R2.csv'.format(outpath, model_name), index=False)
    print('')

    # explanations
    for m, model_name in enumerate(['GBM']): 
        exp = pd.DataFrame(feature_explanations[m])
        fold = pd.DataFrame(fold.astype(int), columns=['fold'])
        feat_exp = pd.concat((subject_data, fold, exp), axis=1)
        print('model explanations: {:}revision_shap/{:}-model-feature-explanations.csv'.format(outpath, model_name))
        print('')
        feat_exp.to_csv('{:}revision_shap/{:}-model-feature-explanations.csv'.format(outpath, model_name), index=False)

        # save for later CV models
        print('model explanations for cross-validation: {:}revision_shap/{:}-model-all-fold-feature-explanations.npy'.format(genpath, model_name))
        np.save('{:}revision_shap/{:}-model-all-fold-feature-explanations.npy'.format(genpath, model_name), fold_feature_explanations[m,:,:,:])
        print('model predictions for cross-validation: {:}revision_shap/{:}-model-all-fold-delta.npy'.format(genpath, model_name))
        np.save('{:}revision_shap/{:}-model-all-fold-delta.npy'.format(genpath, model_name), fold_predictions[m,:,:])
