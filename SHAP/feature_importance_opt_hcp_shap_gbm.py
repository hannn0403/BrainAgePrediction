...
def main():
    ...
    opt_gbm_feature_num = 49
    hcp_gbm_features = hcp_gbm_feat_list[:opt_gbm_feature_num]
    hcp_gbm_features.append('Age')
    hcp_gbm_features.append('Subject')

    subject_data = subject_data.loc[:, hcp_gbm_features]
    #####################################################################################################
    # K-FOLD
    #####################################################################################################
    # 데이터 셋을 변경하는 경우 이 부분 변경 필요 
