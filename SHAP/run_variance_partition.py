...
def main():
  ...
  for model in ['linear', 'nonlinear', 'ensemble']:
      ...
      # metadata
      subject_info = model_explanations.loc[:, model_explanations.columns.isin(['ID', 'Age', 'Sex' 'fold'])]
      # feature importances
      explanations =  model_explanations.loc[:, ~model_explanations.columns.isin(['ID', 'Age', 'Sex', 'fold'])]
      # brain age deltas
